    ...
    class WishpondPlugin {
        ...
        if (isset($_POST['submit']) && $_POST['submit']) {
          if (isset($_POST['token'])) {
            ...
            if ($token === get_option('wishpond_auth_token')) {
              $error_message = __('Token value is the same: ', 'wishpond-social-campaigns') . $token;
            } else {
              $saved = update_option('wishpond_auth_token', $token, '', 'no');

              if ($saved) {
                $notice_message = __('Successfully updated token: ', 'wishpond-social-campaigns') . $token;
              } else {
                $error_message = __('Unable to update token: ', 'wishpond-social-campaigns') . $token;
              }
            }
          }
        }
      ...
      public function publish_campaign($data) {
        if (!is_user_logged_in()) {
          wp_die(__('Not logged in', 'wishpond-social-campaigns'));
        }

        if (!current_user_can('activate_plugins')) {
          wp_die(__('Not enough permissions', 'wishpond-social-campaigns'));
        }

        if (!WishpondUtilities::permalink_structure_valid()) {
          $message = __('Invalid permalink structure. Please go to "Settings"->"Permalinks" and make sure your permalinks use the "postname"', 'wishpond-social-campaigns');
          ...
          $action   = __('saved', 'wishpond-social-campaigns');

          if ($campaign->post_id !== -1) {
            $saved  = $campaign->update($path);
            $action = __('updated', 'wishpond-social-campaigns');
          } else {
            $saved = $campaign->save();
          }
          ...
          if ($saved) {
            ...
            $message = WishpondUtilities::json_message('updated', __('Campaign ', 'wishpond-social-campaigns') . $action . __(' successfully. ', 'wishpond-social-campaigns') . $campaign_url);
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'wishpond-social-campaigns'));
          }
        }
      ...
      public function delete_campaign($data) {
        ...
        if ($campaign) {
          if ($campaign->remove()) {
            $message = WishpondUtilities::json_message('updated', __('Campaign removed successfully.', 'wishpond-social-campaigns'));
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'wishpond-social-campaigns'));
          }
        } else {
          $message = WishpondUtilities::json_message('error', __('Campaign not found.', 'wishpond-social-campaigns'));
        }
      ...
      function wishpond_load_plugin_textdomain() {
        load_plugin_textdomain( 'wishpond-social-campaigns', FALSE, basename( dirname( __FILE__ ) ) . '/lang/' );
      }
    }
  }
